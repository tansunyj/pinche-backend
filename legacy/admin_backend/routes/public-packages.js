...
function parseModelsField(val) {
  ...
  try {
    ...
    console.error('[public-packages] models JSON parse error:', e.message);
    return [];
  }
}
...
/**
 * GET /api/packages
 * 公开套餐列表（无需登录），仅返回启用且未删除的套餐
 */
router.get('/', async (req, res) => {
  try {
    const { page = 1, pageSize = 20 } = req.query;

    const whereClause = 'WHERE deleted_at IS NULL AND status = 1';
    ...
    console.error('[public-packages] list error:', error);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!row) {
      return res.status(404).json({ success: false, error: '套餐不存在或已下架' });
    }
    ...
    console.error('[public-packages] detail error:', error);
...
router.post('/:id/activate', authMiddleware, async (req, res) => {
  try {
    ...
    const [pkg] = await query(
      `SELECT id, name, status, start_at, end_at, min_consumption, max_consumption
    ...
    if (pkg.status !== 1) {
      return res.status(400).json({ success: false, error: '该套餐已下架，无法开通' });
    }

    // 检查套餐是否已过期
    if (pkg.end_at && new Date(pkg.end_at) < new Date()) {
      return res.status(400).json({ success: false, error: '该套餐已过期，无法开通' });
    }
    ...
    if (existing && existing.package_id === packageId) {
      return res.json({ success: true, message: '您已开通该套餐', alreadyActivated: true });
    }
    ...
    try {
      ...
      console.error('[public-packages] 缓存清除失败:', cacheErr.message);
    }

    // 审计日志
    req.audit = {
      action: existing ? 'package.change' : 'package.activate',
      targetType: 'user_package',
      targetId: userId,
      after: { user_id: userId, package_id: packageId, package_name: pkg.name }
    };
    ...
    console.error('[public-packages] activate error:', error);
