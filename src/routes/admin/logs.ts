  ...
  try {
    ...
    res.json({
      logs: (logRows as any[]).map((l) => ({
        ...
        discountRideIds: l.discount_ride_ids ?? null,
        originalQuota: Number(l.original_quota) || 0,
        savedQuota: Math.max((Number(l.original_quota) || 0) - (Number(l.quota_consumed) || 0), 0),
